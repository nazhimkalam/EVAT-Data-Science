...
# Model-specific confidence scores from evaluation
XGBOOST_CONFIDENCE = 0.91  # From GPU-accelerated evaluation on CHARGED dataset
SARIMAX_CONFIDENCE = 0.83  # From GPU-accelerated evaluation on CHARGED dataset
...
def predict_demand_with_model(feature_df: pd.DataFrame) -> Tuple[Optional[float], Optional[float]]:
    """
    Predict demand using loaded model (XGBoost or SARIMAX)

    Returns:
        Tuple of (predicted_demand, uncertainty/confidence)
        - For XGBoost: uncertainty is None (use fixed confidence 0.91)
        - For SARIMAX: uncertainty is standard error
    """
    if DEMAND_MODEL is None:
        return None, None

    try:
        # Check model type and use appropriate prediction method
        model_type = type(DEMAND_MODEL).__name__

        if model_type == "XGBRegressor":
            # XGBoost: use predict() method
            X_values = feature_df.values
            predicted = float(DEMAND_MODEL.predict(X_values)[0])
            # XGBoost doesn't provide uncertainty, return None
            # Confidence will be computed using fixed value (0.91 from evaluation)
            uncertainty = None
            return predicted, uncertainty

        elif model_type in ["SARIMAXResultsWrapper", "SARIMAX"]:
            # SARIMAX: use get_forecast() method
            ...
            # Unknown model type, attempt generic predict
            try:
                predicted = float(DEMAND_MODEL.predict(feature_df.values)[0])
                return predicted, None
            except Exception:
                # Fallback to forecast for SARIMAX-like models
                forecast = DEMAND_MODEL.get_forecast(steps=1, exog=feature_df)
                predicted = float(forecast.predicted_mean.iloc[0])
                return predicted, None
...
def _compute_demand(suburb: str, cluster: int, lag_usage: float, dt: datetime) -> Dict[str, object]:
    ...
    if predicted_demand is None or predicted_demand < 0:
        ...
        # Use model-specific confidence scores if available
        if DEMAND_MODEL is not None:
            model_type = type(DEMAND_MODEL).__name__
            if model_type == "XGBRegressor":
                confidence_score = XGBOOST_CONFIDENCE  # 0.91 from evaluation
            elif model_type in ["SARIMAXResultsWrapper", "SARIMAX"]:
                # Use uncertainty-based calculation for SARIMAX
