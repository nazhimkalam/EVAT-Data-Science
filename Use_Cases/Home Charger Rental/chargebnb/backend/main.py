...
def startup():
    global clustered_df, prices_df, chargers_df, coords_df

    print("\n" + "="*60)
    print("CHARGEBNB BACKEND STARTUP")
    print("="*60)
...
def get_listings(start: str | None = None, end: str | None = None):
    ...
    for _, row in prices_df.iterrows():
        ...
        optimal_price = float(row["Average_Price"])
...
def get_pricing(suburb: str, start: str | None = None):
    ...
    base_price = float(row["Average_Price"])
    lag_usage = float(row.get("Lag_Usage", row["Average_Price"]))
...
def create_booking(payload: BookingRequest):
    print(f"\n=== Creating booking for {payload.suburb} ===")

    # Ensure bookings table exists (safety check)
    try:
        init_db()
    except Exception as e:
    ...
    print(f"✓ Booking created: ID={booking_id}, Suburb={payload.suburb}, Amount=${booking_details['backend_expected_total']}")
