...
**ev_charging_clustering_pipeline.py**
```python
import pandas, numpy, matplotlib.pyplot, folium
...
1. **Data Paths** - Scripts expect CSV files in current working directory
2. **Synthetic Data** - Usage is generated data, not real bookings
3. **Reproducibility** - KMeans uses random_state=42, but NN training may vary slightly
4. **File Naming** - Script reads `clustered_suburbs (1).csv` with parentheses
5. **Computational Requirements** - Runtime ~2-3 minutes total, no GPU required
6. **Data Assumptions** - Melbourne suburbs only, 6-month simulation period, $5-$10 price range
